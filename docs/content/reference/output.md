...
describes what users get. Pick a format with `-o`, or let aviationweather choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
aviationweather <command> -o table   # aligned columns for reading
aviationweather <command> -o jsonl   # one JSON object per line, for piping
aviationweather <command> -o json    # a single JSON array
aviationweather <command> -o csv     # spreadsheet friendly
aviationweather <command> -o tsv     # tab-separated
aviationweather <command> -o url     # just the URL column
aviationweather <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
aviationweather <command> --fields id,title,url
```
...
```bash
aviationweather <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
aviationweather <command>            # a table, because this is a terminal
aviationweather <command> | wc -l    # JSONL, because this is a pipe
```
