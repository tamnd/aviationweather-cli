// Package cli assembles the aviationweather command tree from the aviationweather
...
// NewApp assembles the kit application from the aviationweather domain. The
...
//
// To add a command, declare it in aviationweather/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := aviationweather.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(aviationweather.Domain{}).Register(app)
